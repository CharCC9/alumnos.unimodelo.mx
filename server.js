    ...
    return `
        ...
        <body>
            <div class="login-card">
                <div class="logo-container"><img src="https://via.placeholder.com/110" alt="Logo Universidad Modelo"></div>
    ...
    return `
        ...
        <head>
            ...
            <style>
                ...
                .custom-select-trigger { background-color: white !important; color: #333 !important; border: 1px solid #ccc; border-radius: 3px; height: 34px; line-height: 34px; padding: 0 30px 0 10px; text-transform: none; font-size: 14px; font-weight: 400; display: inline-flex; align-items: center; justify-content: space-between; width: 190px; position: relative; box-shadow: none !important; margin-top: 15px;}
                ...
                .custom-menu-li.active-item a { color: #0d47a1 !important; font-weight: bold !important; }
                .custom-menu-li.active-item a i.menu-arrow { color: #0d47a1; }

                .collapsible-header { background-color: transparent !important; border: none !important; padding: 14px 20px !important; font-size: 13px !important; font-weight: 500 !important; color: #444 !important; text-transform: uppercase; display: flex !important; align-items: center; justify-content: space-between; cursor: pointer;}
                ...
                .collapsible-body { padding: 0 !important; border: none !important; background-color: #fafafa; display: none; }
        ...
        <body>
                    ...
                    <nav class="navbar-color">
                        <div class="nav-wrapper">
                            <div class="nav-brand-area">
                                <a href="#" style="color:white; display: flex; align-items: center;" id="toggle-sidebar" class="waves-effect waves-light">
                                    <i class="material-icons" style="font-size:28px;">menu</i>
                                </a>
                                <img src="https://via.placeholder.com/44" alt="Escudo Universidad Modelo">
                            </div>
                            ...
                            <div style="display: inline-block; margin-left: 20px; vertical-align: top;">
                                ...
                                <span style="font-size: 22px; color: white; margin-left: 20px; vertical-align: middle; font-weight: 300; line-height: 64px;">Universidad Modelo 🏆</span>
                            </div>
                            <ul class="right hide-on-med-and-down" style="margin-right: 20px;">
                                <li style="color: white; font-size: 14px; display: inline-block; vertical-align: middle; margin-right: 10px; line-height: 64px;">SANTIAGO DE JESUS ARCOS GUZMAN</li>
            ...
            <div id="main" class="mainPaddingSidebar">
                <div class="wrapper">
                    <aside id="left-sidebar-nav">
                        <ul class="collapsible collapsible-accordion" style="margin: 0; padding: 0; list-style: none;">
            ...
            <script>
                ...
                // --- CIERRE Y COMPLETADO DEL OBJETO SECTIONS DATA ---
                const sectionsData = {
                    libreta_de_pago: {
                    ...
                    calificaciones: {
                        ...
                        html: '<h5 style="font-weight: 400; color: #222; text-transform: uppercase;">Calificaciones del Alumno</h5>' +
                              ...
                              '<p style="margin: 5px 0 20px 0;"><b>Nombre:</b> SANTIAGO DE JESUS ARCOS GUZMAN</p>' +
                              '<table class="simulated-table">' +
                              '<thead><tr><th>Materia</th><th>Parcial 1</th><th>Parcial 2</th><th>Parcial 3</th><th>Final</th></tr></thead>' +
                              '<tbody>' +
                              '<tr><td>ALGORITMOS</td><td>90</td><td>85</td><td>95</td><td>90</td></tr>' +
                              '<tr><td>CALCULO DIFERENCIAL</td><td>70</td><td>80</td><td>75</td><td>75</td></tr>' +
                              '<tr><td>FISICA APLICADA</td><td>88</td><td>92</td><td>85</td><td>88</td></tr>' +
                              '</tbody></table>' + footerCalificaciones
                    },
                    extra_inscritos: {
                        label: 'Exámenes Inscritos',
                        breadcrumb: 'Inicio <i class="material-icons">chevron_right</i> Extraordinarios <i class="material-icons">chevron_right</i> Inscritos',
                        html: '<h5>Exámenes Extraordinarios Inscritos</h5><p>No cuentas con exámenes extraordinarios inscritos en este periodo.</p>'
                    },
                    extra_calificaciones: {
                        label: 'Calificaciones Extraordinarios',
                        breadcrumb: 'Inicio <i class="material-icons">chevron_right</i> Extraordinarios <i class="material-icons">chevron_right</i> Calificaciones',
                        html: '<h5>Calificaciones de Extraordinarios</h5><p>No se registran calificaciones de extraordinarios.</p>'
                    },
                    micuenta: {
                        label: 'Mi Cuenta',
                        breadcrumb: 'Inicio <i class="material-icons">chevron_right</i> Mi Cuenta',
                        html: '<h5>Mi Cuenta</h5><p><b>Alumno:</b> SANTIAGO DE JESUS ARCOS GUZMAN</p><p><b>Carrera:</b> Ingeniería en Sistemas Computacionales</p>'
                    }
                };
                ...
                function showSection(sectionKey) {
                    ...
                    setTimeout(() => {
                        ...
                        $('#dynamicRenderCard').html(data.html);
                        $('#label-select-actual').html(data.label + ' <i class="material-icons">arrow_drop_down</i>');
                        $('#dynamicRenderCard').css('opacity', 1);
                    }, 200);

                    // Manejar clases activas del menú lateral
                ...
                $(document).ready(function(){
                    ...
                    // Colapsable personalizado para extraordinarios
                    ...
                    $('#toggle-sidebar').click(function(e){
                        ...
                        $('#main').toggleClass('mainPaddingSidebar').toggleClass('mainPaddingLeft');
                    });
...
// Manejo del intento de login
app.post('/login', (req, res) => {
    const { username, password } = req.body;

    // Simulación de credenciales válidas (puedes cambiarlo)
    if (username === 'santiago' && password === 'modelo123') {
        res.redirect('/portal');
    } else {
        // En caso de fallar, vuelve a renderizar el Login mostrando la alerta de error
        res.send(getLoginTemplate(true));
    }
});

// Ruta que sirve el portal del alumno
